...
const SearchPage = () => {
  ...
  const results = mockGems.filter((g) => {
    ...
    <div className="min-h-screen pb-24">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search countries, cities, gems..."
                className="w-full rounded-full bg-secondary py-2.5 pl-10 pr-4 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30"
                autoFocus
              />
            </div>
      ...
      <main className="mx-auto max-w-lg px-4 pt-4">
        ...
        <div className="mt-4 flex items-center justify-between">
          <span className="text-sm text-muted-foreground">{results.length} gems found</span>
          <div className="flex gap-1 rounded-full bg-secondary p-0.5">
            <button
              onClick={() => setView("list")}
              className={`rounded-full p-1.5 transition-all ${view === "list" ? "bg-card shadow-sm" : ""}`}
            >
              <List className="h-4 w-4" />
            </button>
            <button
              onClick={() => setView("map")}
              className={`rounded-full p-1.5 transition-all ${view === "map" ? "bg-card shadow-sm" : ""}`}
            >
        ...
        {view === "list" ? (
          ...
          <div className="mt-4 flex h-[60vh] items-center justify-center rounded-2xl bg-secondary">
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground/60">Connect backend to enable</p>
            </div>
          </div>
        )}
      </main>
